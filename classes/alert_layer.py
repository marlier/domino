...
def check_paging_alerts():
	...
	'''
	return Mysql.query('''SELECT * FROM alerts WHERE ack != 0 AND (NOW() - lastPageSent) > %s and position == 2 ORDER BY id DESC''' % (conf['paging_alert_interval']), "alerts")
	
def check_email_alerts():
	'''
	This returns a list of alerts that needs an notification sent out.
	'''
	return Mysql.query('''SELECT * FROM alerts WHERE ack != 0 AND (NOW() - lastEmailSent) > %s and position == 1 ORDER BY id DESC''' % (conf['email_alert_interval']), "alerts")
